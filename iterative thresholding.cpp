...
void  iterative_thresholding(Mat& rois) {
	...
	for (int i = 0; i < rois.rows; i++) { //从开始寻找
		...
		if ((i + 1) % 9 == 0) {                 //9个一组 算法开始  对于这里为什么选9是一个个调的找到最佳分组，如果分组小故然精确但会出现无法拟合的现象
			count++;                            // 分组还可以选7 等等
			vector<float> R1;
			vector<float> R2;
			float avrag1 = 0;
		...
		auto a = rois.rows % 9;
		if (i == rois.rows - 1 && a != 0) {     //对于最后不满9个一组的   算法漏洞当只剩1个像素的时候又一点点问题，想着可以直接把最后一个用前面的拟合值代替不用计算
			vector<float> R1;
			vector<float> R2;
			float avrag1 = 0;
			float avrag2 = 0;
